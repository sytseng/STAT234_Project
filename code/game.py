...
def checkers_features_augmented(state, action):
    """
    ...
    # Advantage
    advantage = (agent_pawns_n - oppn_pawns_n)
    advantage_king = (agent_kings_n - oppn_kings_n) 
    
    # next state (from opponent's point of view)
    next_state_oppn = state.generate_successor(action, True)
    
    # piece location
    agent_piece_loc = next_state.board.get_piece_locations()
    oppn_piece_loc = next_state_oppn.board.get_piece_locations()                 
    
    # veritcal centroids location
    if len(agent_piece_loc) != 0:
        agent_vert_center_loc = np.mean(np.array(agent_piece_loc)[:,0]).round()
    else: agent_vert_center_loc = -1
        
    if len(oppn_piece_loc) != 0:
        oppn_vert_center_loc = np.mean(np.array(oppn_piece_loc)[:,0]).round()
    ...
    # Advancement: +1 for each agent's piece in 5 & 6 rows, -1 for each in 3 & 4 rows
    if state.is_first_agent_turn():
        advancement = np.sum(post_action_spots[[4,5],:]==agent_sym)+np.sum(post_action_spots[[4,5],:]==agent_king_sym) \
        - np.sum(post_action_spots[[2,3],:]==agent_sym)+np.sum(post_action_spots[[2,3],:]==agent_king_sym)
    else:
        advancement = np.sum(post_action_spots[[2,3],:]==agent_sym)+np.sum(post_action_spots[[2,3],:]==agent_king_sym) \
        - np.sum(post_action_spots[[4,5],:]==agent_sym)+np.sum(post_action_spots[[4,5],:]==agent_king_sym)

    # Center and XCenter control: +1 for each agent's piece at center location
    center_control = np.sum(post_action_spots[2:-2,1:-1]==agent_sym)+np.sum(post_action_spots[2:-2,1:-1]==agent_king_sym)
    xcenter_control = np.sum(post_action_spots[2:-2,1:-1]==oppn_sym)+np.sum(post_action_spots[2:-2,1:-1]==oppn_king_sym)
    
    king_center_control = np.sum(post_action_spots[2:-2,1:-1]==agent_king_sym)
    xking_center_control = np.sum(post_action_spots[2:-2,1:-1]==oppn_king_sym)

    # Double diagonal and exposure  
    double_diagonal = 0
    exposure = 0
    for loc in agent_piece_loc:
        forward_location = next_state.board.forward_n_locations(loc,1)
        if sum([len(fl)>0 for fl in forward_location])==2:  
            double_diagonal += 1
            if sum([next_state.board.get_spot_info(fl)==next_state.board.EMPTY_SPOT for fl in forward_location])==2:
                exposure += 1

    # Mobility
    mobility = len(next_state.board.get_possible_next_moves())
    mobility_oppn = len(next_state_oppn.board.get_possible_next_moves())
    ...
    # new features
    features.extend([next_state_oppn.num_attacks(),
                     advantage, 
                     num_agent_pawns_edge,
                     num_agent_kings_edge,
                     num_oppn_pawns_edge,
                     num_oppn_kings_edge,
                     num_agent_pawns_center,
                     num_agent_kings_center,
                     num_oppn_pawns_center,
                     num_oppn_kings_center,
                     agent_vert_center_loc,
                     oppn_vert_center_loc,
                     back_row_bridge, 
                     advancement, 
                     center_control, 
                     xcenter_control, 
                     king_center_control, 
                     xking_center_control,
                     double_diagonal, 
                     exposure,
                     mobility,
                     mobility_oppn])
...
def checkers_features_simple(state, action):
    """
    state: game state of the checkers game
    action: action for which the feature is requested
